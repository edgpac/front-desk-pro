...
**Account access:**
- [ ] Remove `owner-gate.ts` and its checks in `login.tsx`/`signup.tsx`/`use-auth.ts` (plus the `VITE_SITE_OWNER_EMAIL` env var) — right now no one but the site owner can create a real account at all, independent of the WhatsApp status above
