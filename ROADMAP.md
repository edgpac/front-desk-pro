...
old `frontdesk.tools` placeholder. **Still open**: `SITE_URL` is not yet
set in Vercel's production environment variables at all — `stripe-server.ts`
currently falls back to `http://localhost:8080` for Checkout redirect URLs
in production, a real bug, not just a stale placeholder — and the
`legal@`/`privacy@frontdesk.tools` mailto addresses in the legal pages
haven't moved to `@jobitready.com` pending confirmation that mailboxes
actually exist there. Also untouched: `package.json`'s
...
| Meta WhatsApp Embedded Signup (`ConnectWhatsAppMeta.tsx`, `meta-whatsapp-server.ts`) | 🔴 blocked, stuck | Stage 2B code is implemented and deployed, not the concierge-only Twilio path described in the row above. One real root cause was found and fixed this pass: Meta's `/oauth/access_token` exchange requires the exact, SDK-internal `redirect_uri` the popup used to open — a dynamic `xd_arbiter` URL Meta's JS SDK generates internally and never exposes through `FB.login()`'s callback, undocumented in Meta's official Embedded Signup docs. Confirmed via live A/B testing against an isolated diagnostic page (now captured client-side via a `window.open()` intercept in `ConnectWhatsAppMeta.tsx` and threaded through to the server-side exchange). That part works — token exchange no longer fails with `error_subcode=36008`. **Currently stuck on the next step, unresolved as of now**: `resolveWabaId` fails every real attempt with "Could not determine which WhatsApp Business Account this signup granted access to" — Meta's `debug_token` response never carries a WABA-scoped `granular_scope`. Ruled out: the Embedded Signup Configuration itself (`whatsapp_business_management`/`whatsapp_business_messaging` permissions and the WhatsApp-account asset type are both present, confirmed by inspecting it directly in the Meta dashboard); the test account's role (Casita Armonìa is Administrator on the app). The actual blocker: every real attempt hits a one-click "Continue as Casita Armonìa" shortcut that silently reuses prior consent and skips the WABA/phone-number selection screens entirely, so no WABA is ever actually chosen or created. Tried and failed to clear it: revoking access under Facebook's "Apps and Websites" settings; `auth_type: "reauthenticate"` on `FB.login()` (only forces a password re-entry, does not reset granted permissions/assets — added, then reverted same session). Not yet confirmed: removing the app's grant under Facebook's separate "Business Integrations" settings (where Business Login/Embedded Signup consent is believed to actually live, distinct from "Apps and Websites"). **This is genuinely an open loop right now** — several plausible fixes have been tried and failed, and there is no confirmed path yet to get a real attempt past the shortcut into fresh WABA creation. |
...
4. ~~**Get a real domain and point it at the deployment.**~~ ✅ Done, 2026-09-08 — `jobitready.com`/`www.jobitready.com` are live in Vercel's domain settings, and the sitemap/`robots.txt`/`llms.txt`/`mock-data.ts` now point at `www.jobitready.com`. **Still open**: `SITE_URL` itself is not set in Vercel's production env vars (Stripe Checkout redirects currently fall back to `localhost:8080` in production — a real bug), and the `legal@`/`privacy@frontdesk.tools` mailto addresses haven't moved pending confirmation that `@jobitready.com` mailboxes exist. No JSON-LD placeholder URL was actually found in `index.tsx`'s `SOFTWARE_SCHEMA` — it has no `url` field, so nothing there needed updating.
