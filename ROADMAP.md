...
design, not something to route around; not pursued for now. Also
consolidated the exact remaining steps to finish the *original*
self-serve Meta Cloud API path — see the new "Path to a real self-serve
launch" section near the bottom. Before that (September 21, 2026): Meta's
official WhatsApp Business
Tools MCP server was installed locally in Claude Code (dev/ops tool only,
not part of the app itself; pending a restart + one-time OAuth
authorization before it's usable) — see the Meta WhatsApp Embedded Signup
row under Backend. Before that (September 19, 2026): the Meta WhatsApp
integration's core pipeline went fully live (app Published, a real number
connected and proven end-to-end across multiple job types), plus
connection-failure alerting, the AI no longer going silent on repeat
customers, and the AI no longer inventing prices for anything off the
price sheet — see the same row, and the flagged-leads section under Phase
1.5. Remaining gate: Advanced Access for genuine third-party self-serve
onboarding (also documented there).
...
| | | **Decision (2026-09-19): Twilio's Tech Provider self-serve path above is dropped, not pursued.** Meta is the primary and now-only self-serve WhatsApp channel going forward — Meta's own onboarding (Embedded Signup, Advanced Access) is the one real self-serve path being built out; Twilio's separate ~3-4 week Tech Provider approval process is not being started. The existing Twilio webhook (`api.whatsapp.webhook.tsx`) stays as-is for any tenant manually concierge-connected to it already — nothing is being ripped out — but no further Twilio self-serve engineering (per-tenant subaccounts, sender registration/polling, the dual-channel `whatsapp_connections` design above) is planned. |
...
| | | **Today's finding (2026-09-19): Advanced Access has not been granted, and no App Review submission is visible.** Checked directly in the Meta dashboard → Permissions and features: both `whatsapp_business_management` and `whatsapp_business_messaging` show **"Ready for testing"** (Standard Access), not Advanced Access. Standard Access only allows Embedded Signup to work for Meta accounts that are already admins/testers on this Meta app — it does **not** yet allow a genuine unrelated third party (a real second business, unconnected to the app's own Meta team) to complete Embedded Signup and connect their own number. This is the actual remaining gate on true multi-tenant self-serve onboarding — not a code problem; the OAuth/token-exchange code itself (`completeMetaWhatsAppSignup`) has still never been exercised for real, in either direction. **Decision (2026-09-19): not pursuing the "add a second Meta account as a tester" tactic** — validating the OAuth code under Standard Access this way was ruled a distraction from the actual roadmap, not a real step toward production. **The one real next step, not yet started:** start the Advanced Access App Review submission (demo video, business justification) — an external review process with its own timeline, same shape as Access Verification was. Not blocking anything else in the meantime; the single manually-connected number (`+52 624 159 3182`, below) already proves the pipeline end-to-end, so this only gates opening signup to genuine strangers. |
| | | **Meta WhatsApp Business Tools MCP server installed locally (2026-09-21) — dev/ops tool only, not part of the JIR app.** Per Meta's official docs (`developers.facebook.com/documentation/mcp/whatsapp-business-tools-mcp`), registered `whatsapp_business_tools` (type `http`, endpoint `https://mcp.facebook.com/whatsapp_business_tools`) directly in Claude Code's own config (`~/.claude.json` → `mcpServers`) — the documented `claude mcp add --transport http ...` CLI command wasn't reachable from this environment's shell, so the same JSON entry was added by hand instead, matching the shape of the existing `palmier-pro` HTTP entry. A timestamped backup of the pre-edit config was taken first; confirmed only the one new key was added, nothing else in the file changed. This is **not** wired into any JIR route, webhook, or the `whatsapp_connections`/Embedded Signup code — it's a personal Claude Code tool for inspecting/managing WhatsApp Business assets directly from a terminal session, kept fully separate from the app's own Meta integration. **Not yet usable**: it's OAuth-based, so it needs a Claude Code restart to pick up the new registration, then a one-time browser sign-in (personal Meta login, choose which business/app to grant access to) before any of its tools can be called or before it can see real WABA data. That authorization step is deliberately left for the user to do by hand, not automated. |
| | | **Investigated 2026-09-24: could Meta's own native WhatsApp AI ("Meta Business Agent") run alongside JIR's own AI on a number, or replace it as a cheaper/broader option? Not pursued — ruled mutually exclusive by Meta's own design, not a code limitation.** Prompted by seeing Meta's native AI reply on a *different*, JIR-unconnected number (`+52 612 169 8328`), on Meta's free self-serve tier. Confirmed via Meta's own published Meta Business Agent eligibility rules: a number already running another authorized AI agent (which JIR's Cloud API integration is) is blocked from also onboarding Meta Business Agent — there's no supported way for the two to divide conversations by topic on one number. The only way to use Meta Business Agent at all would be to *replace* JIR's own AI on that number with Meta's, exposing JIR's price sheet as an Enterprise-tier "custom connector" Meta's agent calls into — a fundamentally different product shape, since Meta's model would then compose every customer-facing reply and none of JIR's own validation/anti-hallucination guarantees (the `outside_service_scope`/round-cap/never-invent-a-price work above) would govern what a customer actually sees. Real numbers found: Meta Business Agent bills $2.00/1M tokens (~4-5¢/message) as of 2026-08-01, one consolidated charge for AI + delivery; partner access needs either Tech Provider status (customer pays Meta directly — the track JIR is already mostly approved for) or Solution Partner status (JIR holds a credit line, bills the customer, pays Meta — unconfirmed how much harder this is to obtain). Flagged as a real risk if ever revisited: at that rate, a moderately busy tenant could plausibly cost more in Meta passthrough than JIR's own Solo/Crew subscription price, and JIR has no per-tenant usage metering today (see Rate limiting below — still just a single global counter) to see that coming before an invoice does. Kept here as investigation history, same as the dormant Phase 1.5 decision-engine work — no code changed. |
...
| ~~Owner-only login gate~~ (`src/lib/owner-gate.ts`) | ✅ Removed 2026-09-24 | The temporary gate (added 2026-09-08) that restricted `/login`/`/signup` to a single hardcoded owner email is gone — `owner-gate.ts` deleted, and the checks in `login.tsx`/`signup.tsx`/`use-auth.ts` removed along with the `blocked` UI state each had. `tsc`/build clean. Any real visitor can now sign up and log in for real. The `VITE_SITE_OWNER_EMAIL` env var itself was left in place in Vercel (unused now, harmless) rather than deleted — removing an env var wasn't necessary to land this and wasn't done without being asked. **Known side effect from the original gate, now moot**: nothing to prune — new signups work normally end to end. |
...
## Path to a real self-serve launch (consolidated 2026-09-24)

Everything below was already true, scattered across the rows above — this
just pulls it into one checklist, since "how far to a real, live,
self-serve launch" spans WhatsApp, auth, and billing separately. Nothing
here is new work discovered today except where marked; it's a consolidation.

**Meta Cloud API WhatsApp self-serve** (full detail in the WhatsApp row under Backend):
- [x] Business Verification — Approved
...
**Account access:**
- [x] Remove `owner-gate.ts` and its checks in `login.tsx`/`signup.tsx`/`use-auth.ts` — done 2026-09-24, real signups now work for anyone

**Billing:**
- [ ] Swap `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` from test-mode to live-mode values
- [ ] Create a live-mode Stripe webhook endpoint (separate object/signing secret from the test one already wired)
- [ ] Confirm the Stripe account is activated for real charges
- [ ] Handle `customer.subscription.updated`/`.deleted` so a Stripe-side cancellation actually revokes access (known gap, Phase 1 item 12 above)

**Not required for launch, deliberately not pursued:** the Meta Business
Agent connector idea investigated 2026-09-24 (see the WhatsApp row above)
— a separate, mutually-exclusive product decision, not a launch blocker.
...
12. ~~**Stripe billing.**~~ ✅ Fully proven end-to-end live, 2026-09-20 — not just code, actually tested. Found and fixed a real production gap first: `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` were never set in Vercel despite the checklist claiming this was done, meaning billing was silently non-functional. Created a dedicated "Job It Ready" Stripe account (separate from unrelated existing accounts on the same login) and a sandbox webhook destination pointed at `https://www.jobitready.com/api/stripe/webhook`, listening for `checkout.session.completed` (the only event the handler currently processes — no lifecycle events like renewals/cancellations/failed payments yet, a real gap for later, not today). Ran the actual chain: signed up → chose the Solo plan → real Stripe Checkout → test card `4242...` → webhook fired (200, confirmed in Vercel logs) → billing page correctly shows `status: active`, real payment method, real invoice pulled from Stripe (`getMyBillingInfo`) — not mock data. Confirmed along the way that `stripe-server.ts` defines prices inline (`price_data` at Checkout-session-creation time) rather than referencing pre-made Stripe Products — the Stripe dashboard's Product catalog is decorative, not read by any code path. **Pricing corrected 2026-09-20**: the sandbox test above ran against placeholder `price_data` (`solo: 800`/`crew: 1900` cents, i.e. $8/$19); `PLAN_FEATURES`'s real prices are now `solo: 1999`/`crew: 3999` cents ($19.99/$39.99), matching what's shown on `/pricing`, the homepage, and the billing page — updated in `stripe-server.ts`, `pricing.tsx`, `dashboard.settings.billing.tsx`, and `index.tsx` together so nothing is out of sync. **Known gap, still open**: only `checkout.session.completed` is handled — `customer.subscription.updated`/`.deleted` aren't, so a cancellation made directly in Stripe won't flip a user's plan back to inactive in Supabase yet. **Also confirmed still open, 2026-09-24**: everything above was proven in Stripe **test mode** (a sandbox webhook destination, test card `4242...`) — going live for real subscribers still needs `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` swapped to live-mode values, a separate live-mode webhook endpoint created (Stripe keeps test/live webhooks as distinct objects with their own signing secrets, not a mode flag on one endpoint), and the Stripe account itself confirmed activated for real charges.
13. ~~**Plan entitlements (Solo vs. Crew feature gating).**~~ ✅ Done, 2026-09-20 — `src/lib/entitlements-server.ts` is the single source of truth (`PLAN_FEATURES`, `getMyPlan`/`myPlanHasFeature`, reads the real `subscriptionStatus`/`plan` off the user's own Supabase auth metadata, never a client-supplied value). First feature gated: CSV lead export, Crew-only. `exportMyLeadsCsv` (`leads-server.ts`) checks the entitlement server-side before running any query — the UI on `/dashboard/leads` showing a real "Export CSV" button (Crew) vs. an "Upgrade" link (Solo) is a courtesy, not the actual protection. Scoped tightly on purpose, per explicit instruction: core AI/WhatsApp/pricing capabilities are not gated by plan. **Not yet live-tested in a browser** — `tsc`/build verified clean, but no one has actually clicked "Switch to Crew" and confirmed a real CSV downloads.
