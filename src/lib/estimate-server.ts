...
Service call fee: $${input.serviceCallFee} — this applies ONLY to time/work that has no specific matching item on the price sheet above. It is a separate, additional concept from a specifically-priced item, never a discount, substitute, or absorption of one: a matched price-sheet item's configured price is always charged in full, on top of the service call fee, even if that item's own work would fit inside the first hour the service call fee already covers. Only hours of genuinely UNMATCHED work beyond the first hour are billed at $${input.laborRate}/hr — this hourly rate is never applied to, and never replaces the price of, work that already has its own specific price-sheet item.
...
2. Once you have enough information, identify every distinct task or issue described, and check EACH one against the price sheet separately — never let an early match, or the service call fee's "first hour," become a blanket assumption that other matched tasks are already covered or free. For each task that reasonably matches a price-sheet item (the same kind of job, even if the exact quantity/scope differs — e.g. "3 outlets" against a per-outlet price is fine): (a) charge that item's own configured price IN FULL — never $0, never a reduced amount, and never treated as "included" in the service call fee, regardless of whether the work would fit within the first hour; (b) word the line item's description so it's clearly recognizable as that specific price-sheet task — use its own name/wording, don't invent a differently-labeled line that merely happens to land on a similar number. Never substitute the generic hourly labor rate for a task that has its own specific listed price, even if the labor-rate math would come out cheaper or the task seems quick — the labor rate is ONLY for genuinely extra work that has no price-sheet item of its own. If nothing on the price sheet reasonably covers what's being asked — a genuinely different kind of job the business hasn't priced at all — respond with {"needsClarification": false, "outOfScope": true} instead of guessing a number. Never estimate a price for something with no reasonable match on the price sheet, even using the labor rate.

Example: price sheet has "Quick fix / minor repair — $60 [BUNDLEABLE]" (keywords include doorknob) and a separate "Toilet / sink / tub unclogging — $60" item. A request describing both a clogged drain and a broken doorknob produces TWO full-price line items — "Toilet / sink / tub unclogging" at its full $60 and "Quick fix / minor repair" at its full $60 — neither one reduced to $0 or folded into the service call fee just because the combined work might fit within the first hour.
...
4. Only include line items that make sense for what was described — don't pad the estimate, and don't drop a real charge to $0 either. Price-sheet items marked [BUNDLEABLE] represent a flat, quick-fix-style visit charge: if the customer describes more than one separate issue that all match the SAME bundleable item, charge that item's full configured price exactly ONCE, not once per issue mentioned (e.g. a loose doorknob and a squeaky hinge both matching one "Quick fix" bundleable item is a single $60 line item, not two $60 line items). Bundling controls how many times an item is charged — never whether it's charged. $0 is never a valid price for a bundleable item any more than for any other matched item. Only add a second charge alongside it if a genuinely different item — non-matching, or not itself bundleable — is also needed.
