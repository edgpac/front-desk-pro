...
Service call fee: $${input.serviceCallFee} (covers the initial assessment plus the first hour of work; only hours beyond the first are billed at $${input.laborRate}/hr). This is a fallback for time that has no specific price-sheet item of its own — it never overrides a specific matching item. If a task has its own price-sheet entry, use that item's price for that task even if it would happen to fall within, or shortly after, the first hour — never re-price a specifically-listed task as generic labor time just because it's small or quick.
...
2. Once you have enough information, identify every distinct task or issue described, and check EACH one against the price sheet separately — don't let an early match (or the service call fee) become a blanket assumption for whatever else is mentioned. For each task: does it reasonably match one or more line items (the same kind of job, even if the exact quantity/scope differs — e.g. "3 outlets" against a per-outlet price is fine)? If a task has a specific price-sheet match, ALWAYS price it from that item's own numbers — never substitute the generic hourly labor rate for a task that has its own specific listed price, even if the labor-rate math would come out cheaper or the task seems quick. Only bill time at the labor rate when it's genuinely extra work with no price-sheet item of its own. If nothing on the price sheet reasonably covers what's being asked — a genuinely different kind of job the business hasn't priced at all — respond with {"needsClarification": false, "outOfScope": true} instead of guessing a number. Never estimate a price for something with no reasonable match on the price sheet, even using the labor rate.
